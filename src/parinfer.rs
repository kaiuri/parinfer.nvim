...
fn on_context(result: &mut State<'_>) -> Result<()> {
    match (result.context, result.ch) {
        ...
        (In::Code, "`") if result.janet_long_strings_enabled => {
        ...
        (In::Code, _) => (),
        (In::Comment, ch) if result.string_delimiters.contains(&ch.to_string()) => {
            in_comment_on_quote(result)
        }
        ...
        (In::Comment, _) => (),
        (In::String { delim }, ch) if delim == ch => {
            result.context = In::Code;
        }
        (In::String { .. }, _) => (),
        ...
        (In::LispReaderSyntax, "!") if result.guile_block_comments_enabled => {
            result.context = In::GuileBlockComment;
        }
        (In::LispReaderSyntax, ";") if result.scheme_sexp_comments_enabled => {
            result.context = In::Code;
        }
        ...
        (In::LispBlockCommentPre { depth }, _) => {
        ...
        (In::LispBlockCommentPost { depth: 1 }, "#") => {
            result.context = In::Code;
        }
        ...
        (
            ...
            "`",
        ) if open_delim_len == close_delim_len + 1 => {
            result.context = In::Code;
        }
        (
            In::JanetLongString {
            ...
            result.context = In::JanetLongString {
                open_delim_len,
                close_delim_len: close_delim_len + 1,
            };
        }
...
pub fn process(request: &'_ Request) -> Answer<'_> {
